        ...
        <div class="mb-4 p-5 bg-white rounded-2xl">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2">
                    ...
                    <div id="observation" name="observation" class="w-full p-4 rounded-md border border-gray-400 min-h-32 text-gray-300">
                       <?= $service->service_detail ?? null; ?>
                    </div>
                </div>

                <div class="col-span-2 flex justify-end">
                    <button class="py-2 px-4 flex items-center gap-2 cursor-pointer bg-yellow-300 text-black rounded-full">
            ...
            <?php if(in_array($service->id_type_service, ["4","56"])): ?>
                ...
                <?php if(in_array($userSystem ->type_user, ["DEV","ADM"])): ?>
                        <div class="flex flex-col md:justify-end md:flex gap-3 w-full">
                            ...
                            <form action="<?= url("/editarservicotrabalhador/atendimentosexcluir"); ?>" method="post" class="flex flex-col sm:flex-row gap-4 justify-end">
                            <?= csrf_input(); ?>

                            <input name="id-service" type="hidden" value="<?= $service->id_service ?>">
                            <input name="id-worker" type="hidden" value="<?= $service->id_worker ?>">   
                                <button class="flex items-end gap-3 cursor-pointer border border-red-400 hover:bg-red-500 hover:text-white text-red-500 py-2 px-4 rounded-full transition duration-300">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
